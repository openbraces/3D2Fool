...
class MyDataset(Dataset):
    def __init__(
        ...
        self.device = device
        self.camou_mask = (
            torch.from_numpy(cv2.imread(camou_mask) / 255)
            .to(device)
            .unsqueeze(0)
            .float()
        )
        ...
        self.camou0 = list(self.aux.texture_images.values())[0].to(self.device)[
            None
        ]  # 汽车原表面纹理
        ...
        self.renderer = MeshRenderer(
            ...
            shader=HardPhongShader(
                device=device, cameras=self.cameras, lights=self.lights
            ),
        )
    ...
    def tex_trans(self, camou):
        ...
        for i in range(6):
            camou_row_list = []
            for j in range(6):
                camou1 = T.RandomHorizontalFlip(p=0.5)(
                    camou.permute(0, 3, 1, 2)[0]
                )  # 依概率p水平翻转
    ...
    def frog(self, img, A=0.5, beta=0.08):
        ...
        center = [
            row * np.random.rand(1).tolist()[0],
            col * np.random.rand(1).tolist()[0],
        ]
        center = torch.tensor(center).to(self.device)
        coordinates = torch.stack(
            torch.meshgrid(torch.arange(row), torch.arange(col)), -1
        ).to(self.device)
    ...
    def add_shadow(self, img1, img0, shadow_dimension=5):
        ...
        mask = (
            torch.from_numpy(mask).float().to(self.device).permute(2, 0, 1).unsqueeze(0)
        )
    ...
    def generate_random_lines(
        self, no_of_drops=10, slant=0, drop_length=100, drop_width=2
    ):
        drops = []
        for i in range(no_of_drops):  ## If You want heavy rain, try increasing this
            ...
            y = np.random.randint(
                drop_width, self.img_size[0] - drop_length - drop_width
            )
            drops.append((x, y))
        return drops
    ...
    def add_rain(self, img1, img0, no_of_drops=10, drop_length=100, drop_width=2):
        ...
        rain_drops = self.generate_random_lines(
            no_of_drops, slant * 2, drop_length, drop_width
        )
        mask = np.ones([self.img_size[0], self.img_size[1], 3])
    ...
    def phy_trans(self, img1, img0, index):
        ...
        if weather == "w1":  # 晴朗
            # img1, img0 = self.Color_trans(img1, img0, brightness=[0.9, 1.3])
            img1, img0 = self.myColor_trans(
                img1, img0, flag, brightness=[0.9, 1.3]
            )  # 颜色变换
        elif weather == "w2":  # 多云
            # img1, img0 = self.Color_trans(img1, img0, brightness=[0.8, 1.2])
            img1, img0 = self.myColor_trans(
                img1, img0, flag, brightness=[0.8, 1.2]
            )  # 颜色变换
            if map == "Town04":  # 加雾
                A = (
                    dist / 15 * 0.3 + np.random.rand(1).tolist()[0] * 0.06 - 0.03
                )  # 扰动
                beta = (
                    dist / 15 * 0.04 + np.random.rand(1).tolist()[0] * 0.01 - 0.005
                )  # 扰动
            ...
            img1, img0 = self.myColor_trans(
                img1, img0, flag, brightness=[0.7, 1.1]
            )  # 颜色变换
            if map == "Town04":  # 加雾
                A = dist / 15 * 0.5 + np.random.rand(1).tolist()[0] * 0.1 - 0.05  # 扰动
                beta = (
                    dist / 15 * 0.08 + np.random.rand(1).tolist()[0] * 0.02 - 0.01
                )  # 扰动
                img1 = self.frog(img1, A, beta)
                img0 = self.frog(img0, A, beta)
        return img1, img0
    ...
    def set_textures(self, camou):
        ...
        if self.tex_trans_flag:
            image = (
                self.camou0 * (1 - self.camou_mask)
                + self.tex_trans(camou) * self.camou_mask
            )
        else:
            image = (
                self.camou0 * (1 - self.camou_mask)
                + self.tex_trans0(camou) * self.camou_mask
            )
        ...
        self.mesh.textures = TexturesUV(
            verts_uvs=[self.verts_uvs], faces_uvs=[self.faces_uvs], maps=image
        )
    ...
    def __getitem__(self, index):
        ...
        R, T = look_at_view_transform(
            eye=(tuple(eye),), up=(tuple(camera_up),), at=((0, 0, 10),)
        )
        self.cameras = FoVPerspectiveCameras(
            device=self.device, R=R, T=T, znear=1.0, zfar=300.0, fov=45.0
        )
        # create point lights
        self.renderer.shader.lights = PointLights(device=self.device, location=[eye])
        # create materials for rendering
        self.materials = Materials(
            device=self.device, specular_color=[[1.0, 1.0, 1.0]], shininess=500.0
        )
        ...
        self.mesh0.textures = TexturesUV(
            verts_uvs=[self.verts_uvs], faces_uvs=[self.faces_uvs], maps=self.camou0
        )
        ...
        total_img = torch.where((contour == 0.0), img, imgs_pred11)  # [1, 3, 320, 1024]
        total_img0 = torch.where(
            (contour == 0.0), img, imgs_pred00
        )  # [1, 3, 320, 1024]
...
if __name__ == "__main__":
    ...
    dataset = MyDataset(
        data_dir, img_size, obj_name, camou_mask, device=device, phy_trans_flag=True
    )
    ...
    for i, (
        ...
        imgs_pred,
        imgs_pred0,
    ) in enumerate(tqdm_loader):
        index = int(index[0])

        total_img_np = total_img.data.cpu().numpy()[0] * 255
        # print(total_img_np.shape)
        total_img_np = Image.fromarray(
            np.transpose(total_img_np, (1, 2, 0)).astype("uint8")
        )
        ...
        total_img_np0 = Image.fromarray(
            np.transpose(total_img_np0, (1, 2, 0)).astype("uint8")
        )
        total_img_np0.save(os.path.join(log_dir, str(i) + "test_total0.jpg"))
        # print(mask.shape)

        Image.fromarray(
            (255 * imgs_pred).data.cpu().numpy()[0].transpose((1, 2, 0)).astype("uint8")
        ).save(os.path.join(log_dir, str(i) + "img_pred.png"))
        Image.fromarray(
            (255 * imgs_pred0)
            .data.cpu()
            .numpy()[0]
            .transpose((1, 2, 0))
            .astype("uint8")
        ).save(os.path.join(log_dir, str(i) + "img_pred0.png"))
        Image.fromarray(
            (255 * img).data.cpu().numpy()[0].transpose((1, 2, 0)).astype("uint8")
        ).save(os.path.join(log_dir, str(i) + "img.png"))
        Image.fromarray(
            (255 * mask).data.cpu().numpy()[0].transpose((1, 2, 0)).astype("uint8")
        ).save(os.path.join(log_dir, str(i) + "mask.png"))
        if i >= 11:
            break
